//May still need to make changes around line 450?
...
var personElement = function( person ) {
  ...
  el.append($("<span />").addClass("icon rostered").attr("data-checked", "null"));
  el.append($("<span />").addClass("icon technical").attr("data-checked", !person.technical ));

  return el;
};
// Create message to show whether or not a person is on the roster for an event
var updateResultActions = function() {
  results.forEach(function( person ) {
    var message = person.element.find(".message");
    var rostericon = person.element.find(".icon.rostered");
  
    console.log(person);
    //Used to check if the person is rostered for the specific event.
    if( !event ) {
      ...
      rostericon.attr("data-checked", "null");
      person.valid = false;
      person.canForce = false; 
    } else if ( !event.tournament && (event.name != "Lip Sync") ) {
      rostericon.attr("data-checked", "true");
      person.valid = true;
    } else if( (event.volleyball) && (person.volleyball) ) {
      rostericon.attr("data-checked", "true");
      person.valid = true;
      person.canForce = true;
    } else if( (event.dodgeball) && (person.dodgeball) ) {
      rostericon.attr("data-checked", "true");
      person.valid = true;
      person.canForce = true;
    } else if( (event.lipsync) && (person.lipsync) ) {
      rostericon.attr("data-checked", "true");
      person.valid = true;
      person.canForce = true;
    } else if( (event.treds) &&  (person.treds) ) {
      rostericon.attr("data-checked", "true");
      person.valid = true;
      person.canForce = true;
    } else if( (event.trivia) &&  (person.trivia) ) {
      rostericon.attr("data-checked", "true");
      person.valid = true;
      person.canForce = true;
    } else if( (event.basketball) && (person.basketball) ) {
      rostericon.attr("data-checked", "true");
      person.valid = true;
      person.canForce = true;
    } else if( (event.track) &&  (person.track) ) {
      rostericon.attr("data-checked", "true");
      person.valid = true;
      person.canForce = true;
    }else {
      rostericon.attr("data-checked", "false");
      person.valid = false;
      person.canForce = true;
      message.html("In order to check in, " + person.first + " must be on the <strong> Event Roster </strong> roster.");
    }
      

    // Checks everything but event roster.  
    ...
    if( person.gw_role ) {
      ...
      person.valid = false;
      person.canForce = false;
    } else if( person.events.indexOf( String(event.name) ) > -1 ) {
    ...
    } else if( event.waiver && (!person.w_general) ) {
      // Otherwise, if there is an event and the member doesn't have the required waiver, note that
      message.html("In order to check in, " + person.first + " must fill out a <strong>General</strong> waiver.<br />Press [Control] + [Enter] to force a check-in (this will also mark " + person.first + " as having filled out a waiver).");
      person.valid = false;
      person.canForce = true;
      //Not sure about this next else statement yet. Also should this be force enabled.
...
// Constants
//Will need to make changes to how this works if I to include the requirement of being on a roster for tournaments or olympics.

/* Event options: array of string with special format
 *   "<EventName>:[g|l|n][c][t][v|d|f|r|b|z|i|k|u|w|j|m]"
 ...
 *     v -- indicates the event volleyball
 *     d -- indicates the event dodgeball  
 *     z -- indicates the event track
 *     f -- indicates the event treds  
 *     b -- indicates the event basketball
 *     r -- indicates the event trivia
      l -- lipsync waiver
 */
var eventOptions = [
  "Dodgeball:dct",
...
//The changes to the return statement will effect something in the datbase I think?
// Convert eventOptions into JSON
eventOptions = eventOptions.map(function( event, index ) {
  var result = /([A-Za-z0-9 ]+):(v)?(d)?(f)?(r)?(b)?(z)?(l)?(g|n)?(c?)(t?)/.exec( event ),
      w = result[9];
  var waivers = {
    "g": "general",
    "n": false
