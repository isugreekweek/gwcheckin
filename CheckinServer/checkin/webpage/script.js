...
var personElement = function( person ) {
  ...
  el.append($("<span />").addClass("iconrostered").attr("data-checked", 2 ));
...
var updateResultActions = function() {
  results.forEach(function( person ) {
    ...
    var rostericon = person.element.find(".iconrostered");
    ...
    if( !event ) {
      ...
      rostericon.attr("data-checked", 2);
      person.valid = false;
      person.canForce = false;
    } else if( event.rostered && ( !person[event.rostered] )) {
      message.html("In order to check in, " + person.first + " must be on the <strong>" + event.rostered + "</strong> roster.");
    ...
    if( person.events.indexOf( event.name ) > -1 ) {
...
eventOptions = eventOptions.map(function( event, index ) {
  ...
  return {
    ...
    tally:        !!result[4],
    tournament:   !!result[5],
    rostered:  tourna_lip[e] //Which event roster a person must be on.
  };
});
